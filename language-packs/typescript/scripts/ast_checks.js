...
function checkEvalUsage(ast, filePath, sourceLines) {
    const findings = [];
    const now = new Date().toISOString();
    const lang = inferLang(filePath);
    // Detect eval() calls
    ...
    walkAST(ast, (node) => {
        if (node.type === typescript_estree_1.AST_NODE_TYPES.NewExpression &&
...
function checkInnerHtmlAssignment(ast, filePath, sourceLines) {
    const findings = [];
    const now = new Date().toISOString();
    const lang = inferLang(filePath);
    walkAST(ast, (node) => {
        if (node.type !== typescript_estree_1.AST_NODE_TYPES.AssignmentExpression)
            return;
        // Check if left side is .innerHTML or .outerHTML
        const left = node.left;
        if (left.type !== typescript_estree_1.AST_NODE_TYPES.MemberExpression)
            return;
        if (left.property.type !== typescript_estree_1.AST_NODE_TYPES.Identifier)
            return;
...
function checkTestNoAssertion(ast, filePath, sourceLines) {
    const findings = [];
    const now = new Date().toISOString();
    const lang = inferLang(filePath);
    // Only check test files
    ...
    walkAST(ast, (node) => {
        ...
        if (callback.type !== typescript_estree_1.AST_NODE_TYPES.ArrowFunctionExpression &&
            callback.type !== typescript_estree_1.AST_NODE_TYPES.FunctionExpression)
            return;
        // Check if the callback body contains an expect() call
        let hasExpect = false;
        walkAST(callback, (inner) => {
            if (inner.type === typescript_estree_1.AST_NODE_TYPES.CallExpression &&
                inner.callee.type === typescript_estree_1.AST_NODE_TYPES.Identifier &&
                inner.callee.name === 'expect') {
                hasExpect = true;
            }
            // Also check for expect as part of a member expression chain (e.g., expect(...).toBe)
            if (inner.type === typescript_estree_1.AST_NODE_TYPES.CallExpression &&
                inner.callee.type === typescript_estree_1.AST_NODE_TYPES.MemberExpression &&
                inner.callee.object.type === typescript_estree_1.AST_NODE_TYPES.CallExpression &&
                inner.callee.object.callee.type === typescript_estree_1.AST_NODE_TYPES.Identifier &&
        ...
        if (!hasExpect) {
            ...
            if (node.arguments[0].type === typescript_estree_1.AST_NODE_TYPES.Literal && typeof node.arguments[0].value === 'string') {
                testName = node.arguments[0].value;
            }
            else if (node.arguments[0].type === typescript_estree_1.AST_NODE_TYPES.TemplateLiteral && node.arguments[0].quasis.length > 0) {
