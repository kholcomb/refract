...
function checkMagicNumbers(
  ...
  walkAST(ast, (node, ancestors) => {
    ...
    if (TRIVIAL.has(node.value)) return
    ...
    if (parent.type === AST_NODE_TYPES.VariableDeclarator && grandparent) {
      if (
    ...
    if (parent.type === AST_NODE_TYPES.AssignmentPattern) return

    // Context: skip array/object literal values (data, not logic)
    if (parent.type === AST_NODE_TYPES.ArrayExpression) return
    if (parent.type === AST_NODE_TYPES.Property && grandparent?.type === AST_NODE_TYPES.ObjectExpression) return

    // Context: skip computed property access (arr[0], obj[2])
    if (parent.type === AST_NODE_TYPES.MemberExpression && (parent as TSESTree.MemberExpression).computed) return

    // Context: skip bitwise operations (bitmasks are self-documenting)
    if (parent.type === AST_NODE_TYPES.BinaryExpression) {
      const op = (parent as TSESTree.BinaryExpression).operator
    ...
    if (raw && (raw.startsWith('0x') || raw.startsWith('0X') || raw.startsWith('0o') || raw.startsWith('0O'))) return
